...
class OldMainActivity : AppCompatActivity(), View.OnClickListener, AdapterView.OnItemSelectedListener,
    OnMapReadyCallback, GoogleMap.OnCameraIdleListener {

    private val TAG = OldMainActivity::class.java.simpleName
    ...
    private fun checkPermission() {
        ...
        if (permissions.any { ContextCompat.checkSelfPermission(this@OldMainActivity, it) != PackageManager.PERMISSION_GRANTED }) {
            // request for permission
            ActivityCompat.requestPermissions(this@OldMainActivity, permissions, REQUEST_PERMISSION_LOCATION)
        } else {
    ...
    private fun init() {
        ...
        mFusedLocationClient.lastLocation.addOnSuccessListener {
            ...
            Toast.makeText(this@OldMainActivity, "Failed to get last location..", Toast.LENGTH_LONG).show()
            it.printStackTrace()
        }
    }
    ...
    public fun notifyHeartbeat(heartbeat: Heartbeat) {
        ...
        if (!droneMarkers.containsKey(droneId)) {
            ...
            Toast.makeText(this@OldMainActivity, "new marker @ (${position.latitude}, ${position.longitude})", Toast.LENGTH_LONG).show()
    ...
    fun notifyBattery(percentage: Float) {
        ...
        iv_battery.setImageIcon(Icon.createWithResource(this@OldMainActivity, when {
